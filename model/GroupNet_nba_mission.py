...
class DecomposeBlock(nn.Module):
    ...
    def __init__(self, past_len, future_len, input_dim):  # 10,5,288
        ...
        self.decoder_y = MLP(dim_embedding_key + input_dim + 9, future_len * 2, hidden_size=(512, 256), dropout=0.3)
    ...
    def forward(self, x_true, x_hat, f, mission_emb, mission_mask):
        '''
        >>> Input:
            x_true: N, T_p, 2
            x_hat: N, T_p, 2
        ...
        # print(past_embed[0, 0, 0])
        try:
            _, state_past = self.encoder_past(past_embed)
            state_past = state_past.squeeze(0)
        except Exception as e:
            print("Exception occurred in encoder_past:")
            print(f"Error: {e}")
            print(f"past_embed shape: {past_embed.shape}")
            print(f"past_embed dtype: {past_embed.dtype}")
            print(f"past_embed sample: {past_embed.flatten()[:100]}")
            raise


        input_feat_past = torch.cat((f, state_past), dim=1)
        hidden_f = torch.cat((f, mission_emb, mission_mask), dim=-1)
        input_feat_future = torch.cat((hidden_f, state_past), dim=1)

        x_hat_after = self.decoder_x(input_feat_past).contiguous().view(-1, self.past_len, 2)
        y_hat = self.decoder_y(input_feat_future).contiguous().view(-1, self.future_len, 2)
...
class FutureEncoder(nn.Module):
    def __init__(self, args, in_dim=4):
        ...
        self.out_mlp = MLP2(scale_num * 2 * self.model_dim + 9, [128], 'relu')  # 4*2*64 = 512
        self.qz_layer = nn.Linear(self.out_mlp.out_dim, 2 * self.args.zdim)
        initialize_weights(self.qz_layer.modules())

        self.mission_projector = nn.Linear(2 + 1, 8)  # 2 coords + 1 tolerance -> hidden_dim
    ...
    def forward(self, agents_tragets, agents_idx, error_tolerance, inputs, batch_size, agent_num, past_feature):
        length = inputs.shape[1]
        ...
        mission_mask = torch.zeros(batch_size, agent_num, 1,
                                   device=past_feature.device)  # shape: (B, N, 1)
        if agents_idx.numel() != 0:
            mission_tensor = torch.zeros(batch_size, agent_num, 3, device=past_feature.device)  # (B, N, 3)
            mission_tensor[:, agents_idx, :2] = agents_tragets  # inject mission coords
            mission_tensor[:, agents_idx, 2] = error_tolerance  # inject scalar tolerance

            mission_tensor = mission_tensor.view(batch_size*agent_num, -1) # BN, 3
            mission_emb = self.mission_projector(mission_tensor)

            mission_mask[:, agents_idx, :] = 1.0
            mission_mask = mission_mask.view(batch_size * agent_num, -1)  # BN,  1
            mission_emb = (mission_emb * mission_mask)
        else:
            mission_emb = torch.zeros(batch_size, agent_num, 8, device=past_feature.device).view(batch_size*agent_num, -1)
            mission_mask = mission_mask.view(batch_size * agent_num, -1)  # BN,  1



        h = torch.cat((past_feature, final_feature, mission_emb, mission_mask), dim=-1)
...
class Decoder(nn.Module):
    ...
    def forward(self, mission_targets, controlled_idx,error_tolerance, past_feature, z, batch_size_curr, agent_num_perscene, past_traj, cur_location, sample_num,
                mode='train'):
        ...
        mission_mask = torch.zeros(batch_size_curr, agent_num_perscene, 1, device=past_feature.device)  # shape: (B, N, 1)

        if controlled_idx.numel() != 0:
            mission_tensor = torch.zeros(batch_size_curr, agent_num_perscene, 3,
                                         device=past_feature.device)  # (B, N, 3)
            mission_tensor[:, controlled_idx, :2] = mission_targets  # inject mission coords
            mission_tensor[:, controlled_idx, 2] = error_tolerance  # inject scalar tolerance

            mission_tensor = mission_tensor.view(agent_num, -1).unsqueeze(1).expand(-1, sample_num, -1)  # BN, 20, 3
            mission_emb = self.mission_projector(mission_tensor)

            mission_mask[:, controlled_idx, :] = 1.0
            mission_mask = mission_mask.view(agent_num, -1).unsqueeze(1).expand(-1, sample_num, -1)  # BN, 20, 1
            mission_emb = (mission_emb * mission_mask).view(agent_num * sample_num, -1)
        else:
            mission_emb = torch.zeros(batch_size_curr, agent_num_perscene, 8, device=past_feature.device)
            mission_emb = mission_emb.view(agent_num, -1).unsqueeze(1).expand(-1, sample_num, -1)
            mission_emb = mission_emb.reshape(agent_num * sample_num, -1)

            mission_mask = mission_mask.view(agent_num, -1).unsqueeze(1).expand(-1, sample_num, -1)   # BN,20,  1

        mission_mask = mission_mask.reshape(agent_num * sample_num, -1)
        ...
        for i in range(self.num_decompose):  # numdecompose = 2
            x_hat, y_hat = self.decompose[i](x_true, x_hat, hidden, mission_emb, mission_mask)  # recurrent - here it uses GRUs
            prediction += y_hat
...
class GroupNetM(nn.Module):
    ...
    def forward(self, data, agents_tragets, agents_idx, error_tolerance):
        device = self.device
        #setting targets:
        ...
        qz_param, _ = self.future_encoder( agents_tragets, agents_idx, error_tolerance, inputs_for_posterior, batch_size, agent_num, past_feature)  # 32*20, 64
        ...
        if self.args.learn_prior:
            ...
            if self.args.ztype == 'gaussian':
                pz_distribution_one = Normal(params=pz_param)
            else:
                ValueError('Unknown hidden distribution!')
        else:
            if self.args.ztype == 'gaussian':
                pz_distribution_one = Normal(mu=torch.zeros(past_feature.shape[0], self.args.zdim).to(past_traj.device),
        ...
        ### use q ###
        # z = qz_sampled
        pred_traj, recover_traj = self.decoder( agents_tragets, agents_idx, error_tolerance, past_feature, qz_sampled, batch_size, agent_num, past_traj, cur_location,
        ...
        diverse_pred_traj, _ = self.decoder(agents_tragets, agents_idx, error_tolerance, past_feature_repeat, pz_sampled, batch_size, agent_num, past_traj,
                                            cur_location, sample_num=20, mode='inference')

        return pred_traj,recover_traj, qz_distribution, pz_distribution_one, diverse_pred_traj

    def step_annealer(self):
        for anl in self.param_annealers:
            anl.step()

    def inference(self, data,agents_tragets, agents_idx, error_tolerance):
        device = self.device
        batch_size = data['past_traj'].shape[0]
        agent_num = data['past_traj'].shape[1]
        ...
        diverse_pred_traj, _ = self.decoder( agents_tragets, agents_idx, error_tolerance,past_feature_repeat, z, batch_size, agent_num, past_traj, cur_location,
    ...
    def inference_simulator(self, data,agents_tragets, agents_idx, error_tolerance):
        device = self.device
