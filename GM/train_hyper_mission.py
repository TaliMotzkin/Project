...
from loss_mission import LossGroupnet
import datetime
...
from utilis import prepare_targets_mission_net
...
parser.add_argument('--model_save_dir', default='GM/saved_models/nba')
...
parser.add_argument('--timing', type=str, default=datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
...
def train(args, train_loader, epoch, device, model, constant):
    model.train()
    total_iter_num = len(train_loader)
    iter_num = 0
    all_loss = 0
    avg_loss_mission= 0
    lossfn = LossGroupnet(args, netG=model)

    for data in train_loader:
        agents_tragets, agents_idx, error_tolerance, _ = prepare_targets_mission_net(constant, data, args.dataset, device, epoch)

        total_loss, loss_pred, loss_recover, loss_kl, loss_diverse, loss_mission, loss_mission_div = lossfn.compute_groupnet_mission_loss( data,  agents_tragets, agents_idx, error_tolerance, epoch)
        ...
        if iter_num % args.iternum_print == 0:
            print(
                ...
                '| loss_mission: {:03f}| loss_mission_div: {:03f}|'
                .format(epoch, args.num_epochs, iter_num, total_iter_num, total_loss.item(), loss_pred, loss_recover,
                        loss_kl, loss_diverse, loss_mission, loss_mission_div))
        iter_num += 1
        all_loss += total_loss.item()
        avg_loss_mission += loss_mission
    ...
    return avg_loss, avg_loss_mission
...
if args.dataset == 'nba':
    ...
    constant = ConstantSDD0 #only neeeded for training buffer - they are the same - 115
    train_set = TrajectoryDatasetSDD(data_dir="datasets/raw/SDD", obs_len= args.past_length, pred_len=args.future_length, skip=1,
    min_ped=1, delim='space', save_path="datasets/sdd/SDD.pt", mode=args.training_type)
elif args.dataset == 'eth':
    constant = return_the_eth_scene(args.scene)

    train_set = TrajectoryDatasetETH(data_dir="datasets/raw/eth", obs_len=args.past_length, pred_len=args.future_length, skip=1, min_ped=1,
...
for epoch in range(args.epoch_continue, args.num_epochs):
    ...
    avg_loss , avg_loss_mission= train(args, train_loader, epoch, device, model, constant)
    print("avg loss", avg_loss, "avg_loss_mission", avg_loss_mission)
    end_time = time.time()
    ...
    if (epoch + 1) % args.model_save_epoch == 0:
        ...
        saved_path = os.path.join(args.model_save_dir, 'mission_aware'+str(int(epoch) + 1)+'_'+ args.scene +str(args.sdd_scene) + '_'+ args.dataset + '_' + args.training_type + '_'+ str(args.seed) + '_' +str(args.how_far)+args.info +'.p')
        torch.save(model_saved, saved_path)

ploting_losses(avg_train_losses, avg_validate_losses, f'plots/training_loss_{args.dataset}_{args.training_type}_{args.seed}.png')
