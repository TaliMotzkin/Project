...
class LossGroupnet:
    def __init__(self, args ,netG=None):
        self.netG = netG
    ...
    def compute_groupnet_mission_loss(self, data,  agents_tragets, agents_idx, error_tolerance, epoch):

        pred_traj,recover_traj, qz_distribution, pz_distribution, diverse_pred_traj = self.netG(data, agents_tragets, agents_idx, error_tolerance)
        ...
        future_traj = data['future_traj'].view(batch_size * agent_num, self.args.future_length, 2).to(self.device).contiguous()
        ...
        loss_mission_div = self.calcualte_mission_loss( best_trajectories, future_traj, agents_tragets,
                                                           agents_idx,
        ...
        total_loss = loss_pred+ loss_recover+ loss_kl +loss_diverse+loss_mission+ loss_mission_div

        return total_loss, loss_pred.item(), loss_recover.item(), loss_kl.item(), loss_diverse.item(), loss_mission.item(), loss_mission_div.item()
    ...
    def temporal_smoothness_loss(self, traj):
        """
        traj: (B, N, T, 2)
        Encourages small changes between steps
        """
        vel = traj[:, :, 1:, :] - traj[:, :, :-1, :]
        return vel.pow(2).mean()

    def time_weighted_mission_loss(self, pred,  mission, agent_idx, weight_decay=0.9):
        """
        pred: B, N, T, 2
        target: B, N, T, 2 (or just use pred if only doing mission)
        mission: N, 2
        agent_idx: list of controlled agents
        """

        B, N, T, _ = pred.shape
        controlled_pred = pred[:, agent_idx, :, :]  # (B, C, T, 2)
        controlled_mission = mission.unsqueeze(0).unsqueeze(2).expand(B, -1, 1, -1)  # (B, C, 1, 2)

        distances = torch.norm(controlled_pred - controlled_mission, dim=-1)  # (B, C, T)

        # Find timestep closest to the mission
        min_t_idx = distances.argmin(dim=-1, keepdim=True)  # (B, C, 1)

        # Build time-based weights: highest at closest timestep
        time_range = torch.arange(T, device=pred.device).view(1, 1, T)
        time_diffs = (min_t_idx - time_range).abs().float()  # (B, C, T)
        weights = 3* weight_decay ** time_diffs  # e.g., 1, 0.9, 0.81...

        weighted_loss = (distances * weights).mean()  # Final scalar
    ...
    def stretch_gt_to_mission(self, future_traj, mission, mask):
        ...
        for i in idx.tolist():
            ...
            #three knots: at t=0 (start), t=alpha[i] (mission), t=1 (end)
            ...
            # Evaluate each spline at the uniform timesteps
            xs = cs_x(t_uniform)
            ys = cs_y(t_uniform)

            warped = torch.from_numpy(np.stack([xs, ys], axis=1)).to(future_traj.device)
            # *force* the very last point back to the ORIGINAL GT end:
    ...
    def stretch_and_reparam_by_arclength_torch(self, future_traj: torch.Tensor,
        ...
        #compute original path-lengths and alpha
        segs = gt_np[:, 1:, :] - gt_np[:, :-1, :]  # (Bc, T-1, 2)
        seg_lens = np.linalg.norm(segs, axis=-1)  # (Bc, T-1)
        total_len = seg_lens.sum(axis=1)  # (Bc,)
        ...
        #oop only masked trajectories
        for i in np.where(mask_np)[0]:
            # build our 3‐knot spline: start, mission @ alpha[i], end
