...
def train(constant, writer, new_train_loader, args, GM, G, D, criterion):
    ...
    for i in range(args.epoch):
        ...
        for data in new_train_loader:
            ...
            agents_tragets, agents_idx, error_tolerance, _ = prepare_targets_mission_net(constant, data, args.dataset,
                                                                                         args.device, 100, True)

            traj_sample  = data['past_traj'].to(args.device) #B, N, T, 2
            #agents_tragets #B, C, 2


            G.args.sample_k = 1
            prediction_regular, _ = G.inference_simulator(traj_sample) #1, BN, T, 2
            prediction_regular = prediction_regular.detach().to(args.device)

            if args.disc_type in ['groupnet-gan']:
                GM.args.sample_k = 1
                predictions_covert, _ = GM.inference_simulator(traj_sample, agents_tragets,agents_idx,error_tolerance)#1, BN, T, 2
                ...
                predictions_covert = baseline_mission_reach(traj_sample, agents_tragets[0], agents_idx, error_tolerance, args)#1, BN, T, 2
                ...
                predictions_covert = predictions_covert.to(args.device).squeeze(0).view(B, current_N, args.future_length, 2)
                # print("predictions_covert", predictions_covert[0, agents_idx[0]], predictions_covert.shape)
                # print("mission", one_mission[0,agents_idx[0] ])
            ...
            scores_real = D(prediction_regular) #B, N -->> past traj was not given - If it were, the baseline models will be detected immediatly
        ...
        if (i + 1) % args.save_every == 0:
            saveModel_GAN_disc( D, args, str(i + 1))
...
def non_mission_test(test_loader, args, G, D, GM, constant):
    ...
    stats_path = create_stats_folder(args, constant, 'groupnet-gan-disc')
    ...
    for sim_id in range(len(test_loader)):
        ...
        traj_sample, _, _ = test_loader.dataset[
            sim_id]  # traj_sample: [N, T, 2], missions: [N, M, 2], controlled: [max_controlled]
        score_list_sim = []
        ...
        while (future_traj.shape[1] - args.past_length < max_steps):
            with torch.no_grad():
                ...
                fake_traj, _ = GM.inference_simulator(traj_sample, one_mission,
                                                      torch.tensor(agents_idx, device=traj_sample.device),
                                                      mission_tolerance)
...
def mission_test(testing_loader, args, G, GM, D, constant):
    ...
    for sim_id in range(len(testing_loader)):#
        ...
        traj_sample, missions, controlled = testing_loader.dataset[
            sim_id]  # traj_sample: [N, T, 2], missions: [N, M, 2], controlled: [max_controlled]
        ...
        while (future_traj.shape[1] - args.past_length < max_steps):
            with torch.no_grad():
                ...
                if args.disc_type in ['groupnet-gan']:
                    predictions_covert, _ = GM.inference_simulator(traj_sample, one_mission, torch.tensor(agents_idx).to(args.device),mission_tolerance)#1, BN, T, 2

                elif args.disc_type == 'baseline':
                    if len(agents_idx) !=0:

                        # print("agents_idx", agents_idx)
                        # print("one_mission", one_mission.shape)
                        # print("traj_sample shape", traj_sample.shape)

                        predictions_covert = baseline_mission_reach(traj_sample, one_mission,  torch.tensor(agents_idx, device=traj_sample.device), mission_tolerance, args)#1, N, T, 2

                        # print("predictions_covert", predictions_covert[0, agents_idx[0]], predictions_covert.shape)
                        # print("mission", one_mission[0])
                    ...
                    if len(agents_idx) !=0:
                        predictions_covert = baseline_mission_wave(traj_sample, one_mission,  torch.tensor(agents_idx, device=traj_sample.device), mission_tolerance, args)#1, BN, T, 2
                    else:
                        predictions_covert = torch.zeros(1, 0,args.future_length ,2).to(args.device)
        ...
        sum_scores = (scores_tensor * mask).sum(dim=0)  # [N]
        count_valid = mask.sum(dim=0).clamp(min=1)
        sim_scores = sum_scores / count_valid
...
def create_dataset_split_flex(constant, train_loader, save_path,train_size=1000, test_size=100, ):
    ...
    print("past_traj_all_test", past_traj_all_test.shape)
...
if __name__ == '__main__':
    ...
    for name in names:
        ...
        GM = GroupNetM(args, args.device)
        if args.disc_type == 'groupnet-gan':
            GM_path = f"GANG/saved_model/{args.dataset}/{args.saved_models_GAN_GM}.pth" #groupnet-gan
        ...
        test_size = args.sim_num
        save_path = f"GANG/data/train_test_indexes_for_GAN_groupnet_{args.dataset}_{args.training_type}_{args.scene}_{args.sdd_scene}_"
        ...
        if args.mode == 'train':
            ...
            D = TrajectoryClassifier(args.device).to(args.device) #new classifier
            D.to(args.device)
            writer = SummaryWriter(log_dir=f"runs/GAN_testing_disc_groupnet/{args.timestamp}_{args.disc_type}_{args.dataset}_{args.scene}_{args.sdd_scene}_{args.training_type}_{args.seed}_{args.classifier_method}")

            train(constant, writer, new_train_loader, args, GM, G, D, criterion)
            ...
            saved_path = f"GANG/saved_model/{args.dataset}/{args.disc_type}/{args.saved_models_non_trained_DIS}.pth"
