...
from model.models_sampler import SamplerMission
...
def train(train_loader, args, SM, writer, constant,):
    print(args.timestamp)
    SM.train()
    lossfn = LossCompute(args,SM = SM)
    ...
    for i in range(args.epoch):
        ...
        for batch in train_loader:
            ...
            total_loss, pred_loss, score_loss, mission_loss = lossfn.compute_mission_loss(i, agents_tragets,agents_idx,error_tolerance,
                                                   future_traj, past,
                                                   edge_weights_past,
                                                   edge_features_past,
                                                   direction_past,
            ...
            avg_loss_mission += mission_loss


            if iter_num % args.iternum_print == 0:
                print("total_loss ", total_loss.item(),  "mission_loss ", mission_loss.item())

        scheduler_SM.step()
        avg_loss_mission =avg_loss_mission/iter_num
        avg_loss =all_loss/iter_num

        time_end = time.time()

        if (i + 1) % args.save_every == 0:
            saveModel_mission_Sampler(SM, args, str(i + 1))

        print(
            f"Epoch [{i + 1}/{args.epoch}] - "
            f"average mission_loss : {avg_loss_mission} , "
            f"loss_mean: {avg_loss} , "
...
def test_one_traj(folder, test_loader,S, model, args,constant):
    all_num = 0
    l2error_overall = 0
    l2error_dest = 0
    l2error_avg_04s = 0
    ...
    for sim_id in range(len(test_loader)):
        ...
        with torch.no_grad():
            group_net, _ = model.inference_simulator(traj_sample)
            ...
            group_net = group_net.view(20, 1, actor_num, args.future_length, 2).permute(1, 2, 3, 4,
                                                                                            0)  # (B, N, T, 2, 20)


            prediction_new, indexes = S.inference(None, torch.tensor(agents_idx, device=traj_sample.device),current_targets, mission_tolerance, traj_sample,
                                            normalized_vis, velocity, direction, edge_features,edge_weights , group_net)  # prediction: [1, N, T, 2]


        indices_expanded = indexes.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(-1, -1, args.future_length, 2, 1)
        selected = torch.gather(group_net, dim=4, index=indices_expanded).squeeze(-1)

        future_traj = future.unsqueeze(0).detach().cpu().numpy() * args.traj_scale  # B,N,T,2
        batch = future_traj.shape[0]


        selected = selected.squeeze(0).detach().cpu().numpy() # (N,T,2)
        ...
        if args.dataset == "sdd":
            ...
            selected = denormalize_traj(selected)
        ...
        if iteration < 15:
            ...
            previous_3D = np.reshape(normalized_traj_sample.cpu(), (batch, actor_num, args.past_length, 2))
            best = np.reshape(normalized_selected, (batch, actor_num, args.future_length, 2))
        ...
        tcc_new.extend(calc_TCC(selected, future_traj, 1))
...
def non_mission_test(test_loader, args, model, SM, constant):
    random.seed(SEED)
    np.random.seed(SEED)
    torch.manual_seed(SEED)
    torch.cuda.manual_seed_all(SEED)
    stats_path = create_stats_folder(args, constant, "sampler-mission")
    ...
    for sim_id in range(len(test_loader)):#
        ...
        while (future_traj.shape[1] - args.past_length < max_steps):
            with torch.no_grad():
                ...
                normalized_vis, velocity, direction, edge_features, edge_weights = compute_features(args, traj_sample,
                                                                                                    "past")
                prediction_for_infer = prediction.view(20, 1, args.agent_num, args.future_length, 2).permute(1, 2, 3, 4,
                                                                                                             0)  # (B, N, T, 2, 20)

                one_mission = torch.zeros(1, 0, 2, device=args.device)

                eq_in, index = SM.inference(args.alpha, torch.tensor(agents_idx, device=traj_sample.device), one_mission, mission_tolerance, traj_sample,
                                            normalized_vis, velocity,
                                            direction, edge_features, edge_weights, prediction_for_infer)

                indices_expanded_G = index.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(-1, -1, args.future_length,
                                                                                            2, 1)
                selected_G = torch.gather(prediction_for_infer, dim=4, index=indices_expanded_G).squeeze(-1)

                new_traj = selected_G if args.classifier_method == 'sampler_selected' else eq_in  # B, N, T 2
    ...
    if args.dataset == "nba":
        df_centroids.to_csv(f"{stats_path}/no_mission/centroids_agents.csv")
    df_per_sim.to_csv(f"{stats_path}/no_mission/per_sim_stats.csv")
    df_summary.to_csv(f"{stats_path}/no_mission/overall_summary.csv")




def mission_test(test_loader, other_test, args,model, SM, constant):
    random.seed(SEED)
    np.random.seed(SEED)
    torch.manual_seed(SEED)
    torch.cuda.manual_seed_all(SEED)
    ...
    test_one_traj(stats_path, other_test, SM, model, args, constant)
    model.eval()
    SM.eval()
    total_missions = 0
    missions_achieved = 0
    ...
    for sim_id in range(len(test_loader)):#
        ...
        while (future_traj.shape[1] - args.past_length < max_steps):
            with torch.no_grad():
                ...
                agent_missions_ten = torch.tensor(agent_missions, device=traj_sample.device)
                one_mission = agents_targs.gather(1, agent_missions_ten.view(-1, 1, 1).expand(-1, 1, 2)).squeeze(1)

                eq_in, index = SM.inference(args.alpha, torch.tensor(agents_idx, device=traj_sample.device), one_mission,mission_tolerance, traj_sample, normalized_vis, velocity,
                                              direction, edge_features, edge_weights, prediction_for_infer)

                indices_expanded_G = index.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(-1, -1, args.future_length, 2, 1)
                selected_G = torch.gather(prediction_for_infer, dim=4, index=indices_expanded_G).squeeze(-1)

                new_traj = selected_G if args.classifier_method == 'sampler_selected' else eq_in  # B, N, T 2
                # new_traj = best_of_20_mission_baseline(traj_sample, prediction, one_mission, agents_idx,args)
    ...
    "Avg Missions Achieved per Simulation": [missions_achieved / len(test_loader)],
...
def load_dataset(type, test_loader, args, model):
    ...
    if os.path.exists(DATASET_PATH):
        ...
        traj_dataset = torch.load(DATASET_PATH, weights_only=False)  # Load dataset
    else:
...
if __name__ == '__main__':
    ...
    for name in names:
        ...
        if args.mode == 'train':
            ...
            extra_train_loader = load_dataset("train", train_loader, args, model)
            writer = SummaryWriter(log_dir=f"runs/Sampler_mission_training/{args.timestamp}_{args.dataset}_{args.scene}_{args.sdd_scene}_{args.training_type}_{args.seed}_{args.classifier_method}")

            train(extra_train_loader, args,SM, writer, constant)
            ...
            mission_test(testing_loader, other_testing_loader, args, model, SM, constant)
            print("starting non mission test")
            non_mission_test(testing_loader, args, model, SM, constant)
